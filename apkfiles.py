...
class apkfiles_crawler(crawler):
    ...
    def extraction_routine(self, ctr):
        try:
            website = requests.get(self.template + str(ctr), timeout=self.timeout, headers=self.header).text
            ...
            if os.path.exists(self.folder_name + apk_name):
                ...
                apk_bytes = requests.get(self.baseUrl + dl_link, allow_redirects=True, stream=True,
